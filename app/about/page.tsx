    ...
    <div className="enterprise-page-bg enterprise-mesh min-h-screen text-white">
      ...
      <main className="mx-auto max-w-7xl scroll-mt-24 px-4 pb-20 pt-10 md:px-6 md:pt-14">
        ...
        <section className="mb-16 grid grid-cols-2 gap-4 md:mb-20 md:grid-cols-4 md:gap-5" aria-label="Indicatori">
          {[
            { value: "50K+", label: "Anunțuri active", accent: "from-amber-400/90 to-orange-500/90" },
            { value: "100K+", label: "Utilizatori", accent: "from-sky-400/90 to-blue-500/90" },
            { value: "1M+", label: "Vizitatori / lună", accent: "from-emerald-400/90 to-teal-500/90" },
            { value: "4.8★", label: "Rating mediu", accent: "from-violet-400/90 to-fuchsia-500/90" },
          ].map((s) => (
            <div
              key={s.label}
            ...
            >
              <div
                className={`mb-2 bg-gradient-to-br ${s.accent} bg-clip-text text-3xl font-bold tabular-nums text-transparent md:text-4xl`}
              >
                {s.value}
              </div>
        ...
        <section className="mb-16 grid gap-6 md:grid-cols-3 md:gap-8">
          {[
            ...
            <div
              key={item.title}
              className="enterprise-card enterprise-card-hover rounded-2xl p-8 transition-colors hover:border-[rgba(109,91,255,0.25)]"
            >
              <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-[var(--accent-primary)]/25 to-[var(--accent-secondary)]/15 ring-1 ring-white/10">
                {item.icon === "shield" && (
                  <svg className="h-6 w-6 text-[var(--accent-secondary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <section className="enterprise-card rounded-3xl p-10 text-center md:p-12">
          ...
          <Link
            href="/contact"
            className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-dark)] px-8 py-3.5 text-sm font-semibold text-white shadow-[var(--shadow-glow)] transition hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--border-focus)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg-primary)]"
          >
            Contactează-ne
          </Link>
        </section>
