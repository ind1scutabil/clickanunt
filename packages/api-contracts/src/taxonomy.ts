/**
 * Canonical marketplace taxonomy CONTRACT (category + subcategory label/slug tree).
 *
 * This is the shared, client-safe projection of the backend taxonomy
 * (`lib/taxonomy.ts` is the server source of truth incl. attributes + moderation rules).
 *
 * Web and mobile MUST derive their category pickers from this tree so they stay 1:1.
 * A web unit test (`tests/unit/taxonomy-contract-parity.test.ts`) fails the build if
 * `lib/taxonomy.ts` and this contract ever drift.
 *
 * RULES:
 * - Top-level category LABELS are stored verbatim in `Listing.category` — never rename/remove.
 * - Only additions are safe; keep labels + slugs identical to `lib/taxonomy.ts`.
 */
...
export const MARKETPLACE_TAXONOMY: readonly TaxonomyCategoryContract[] = [
  ...
  },
  {
    ...
    slug: 'electronice',
    subcategories: [
      { label: 'Telefoane mobile', slug: 'telefoane' },
      { label: 'Tablete', slug: 'tablete' },
      { label: 'Laptopuri', slug: 'laptopuri' },
  ...
  },
  {
    ...
    subcategories: [
      ...
      { label: 'Servicii medicale', slug: 'servicii-medicale' },
